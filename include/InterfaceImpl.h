...
namespace MFEE {
	...
	public:
		...
		virtual void SetFacialExpressionByType(RE::Actor* a_actor, std::string a_morphType, std::uint32_t morphNumber, float a_value) {
			Mus::ActorManager::GetSingleton().SetMorph(a_actor, Mus::morphNameEntry::GetSingleton().GetMorphTypeByString(a_morphType), morphNumber, a_value);
		};
		virtual void SetFacialExpressionByNumber(RE::Actor* a_actor, std::uint32_t a_morphType, std::uint32_t morphNumber, float a_value) {
			Mus::ActorManager::GetSingleton().SetMorph(a_actor, Mus::morphNameEntry::morphType(a_morphType), morphNumber, a_value);
		};
